...
func init()  {
	...
	orm.RegisterDataBase("default","mysql","root:123456@tcp(127.0.0.1:3306)/src")
	orm.RegisterModel(new(User),new(Address),new(TpshopCategory),new(Goods),new(GoodsImage),new(GoodsSKU),
		new(GoodsType),new(IndexGoodsBanner),new(IndexPromotionBanner),new(IndexTypeGoodsBanner),new(OrderInfo),new(OrderGoods))
